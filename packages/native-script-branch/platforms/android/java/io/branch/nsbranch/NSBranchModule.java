...
public class NSBranchModule {
    ...
    private static final String NS_INIT_SESSION_SUCCESS_EVENT = "NSBranch.initSessionSuccess";
    private static final String NS_INIT_SESSION_ERROR_EVENT = "NSBranch.initSessionError";
    private static final String NS_INIT_SESSION_START_EVENT = "NSBranch.initSessionStart";
    ...
    public NSBranchModule(Context context){
        Log.d("called constructor 2");
        super(context);
        listenForInitSessionEventsToNativeScript(context);
    }
    ...
    public Map<String, Object> getConstants() {
        ...
        // NS events transmitted to JS

        constants.put(INIT_SESSION_SUCCESS, NS_INIT_SESSION_SUCCESS_EVENT);
        constants.put(INIT_SESSION_ERROR, NS_INIT_SESSION_ERROR_EVENT);
        constants.put(INIT_SESSION_START, NS_INIT_SESSION_START_EVENT);
    ...
    private void listenForInitSessionEventsToNativeScript(Context context) {
        mInitSessionFinishedEventReceiver = new BroadcastReceiver() {
            NSBranchModule mBranchModule;

            @Override
            public void onReceive(Context context, Intent intent) {
                final boolean hasError = (initSessionResult.has("error") && !initSessionResult.isNull("error"));
                final String eventName = hasError ? NS_INIT_SESSION_ERROR_EVENT : NS_INIT_SESSION_SUCCESS_EVENT;
                
                mBranchModule.sendNSEvent(eventName, convertJsonToMap(initSessionResult));
            }

            private BroadcastReceiver init(NSBranchModule branchModule) {
                mBranchModule = branchModule;
                return this;
            }
        }.init(this);

        LocalBroadcastManager.getInstance(context).registerReceiver(mInitSessionFinishedEventReceiver, new IntentFilter(NATIVE_INIT_SESSION_FINISHED_EVENT));
        ...
        mInitSessionStartedEventReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (uri != null) {
                    payload.put(NATIVE_INIT_SESSION_STARTED_EVENT_URI, uri.toString());
                }
                else {
                    payload.put(NATIVE_INIT_SESSION_STARTED_EVENT_URI);
                }
                mBranchModule.sendNSEvent(NS_INIT_SESSION_START_EVENT, payload);
            }
        ...
        LocalBroadcastManager.getInstance(context).registerReceiver(mInitSessionStartedEventReceiver, new IntentFilter(NATIVE_INIT_SESSION_STARTED_EVENT));
    }

    @Override
    public void onCatalystInstanceDestroy() {
        Log.d(NATIVE_SCRIPT_CLASS,"onCatalystInstanceDestroy ");

        LocalBroadcastManager.getInstance(getApplicationContext()).unregisterReceiver(mInitSessionFinishedEventReceiver);
        LocalBroadcastManager.getInstance(getApplicationContext()).unregisterReceiver(mInitSessionStartedEventReceiver);
    }
    ...
    public void notifyNativeToInit(){
        Log.d(NATIVE_SCRIPT_CLASS, "notifyNativeToInit");
    ...
    //     response.put(IDENT_FIELD_NAME, ident);
   ...
   //@ReactMethod
    ...
    //         public void onInitFinished(JSONObject referringParams, BranchError error) {
    //             if (error != null) {
    //                 promise.reject("NSBranch::Error::setIdentityAsync failed", error.getMessage());
    //             } else {
    ...
    //             Log.d(NATIVE_SCRIPT_CLASS, "onLinkCreate " + url);
    //             if (error != null) {
    //                 if (error.getErrorCode() == BranchError.ERR_BRANCH_DUPLICATE_URL) {
    //                     promise.reject("NSBranch::Error::DuplicateResourceError", error.getMessage());
    //                 }
    //                 else {
    //                     promise.reject(GENERIC_ERROR, error.getMessage());
    //                 }
    ...
    //@ReactMethod
    public void openURL(String url, Map<String, Object> options) {
        Log.d(NATIVE_SCRIPT_CLASS, "openURL url: " + url);
        if (mActivity == null) {
            // initSession is called before JS loads. This probably indicates failure to call initSession
            // in an activity.
            Log.e(NATIVE_SCRIPT_CLASS, "Branch native Android SDK not initialized in openURL");
            return;
        }
  ...
  //@ReactMethod
    ...
    //     try {
    //         qrCode.getQRCodeAsData(getApplicationContext().getCurrentActivity(), branchUniversalObject, linkProperties, new BranchQRCode.BranchQRCodeDataHandler() {
    //             @Override
    ...
    public BranchQRCode createBranchQRCode(Map<String, Object> branchQRCodeSettingsMap) {
        BranchQRCode branchQRCode = new BranchQRCode();

        if (branchQRCodeSettingsMap.containsKey("codeColor")) branchQRCode.setCodeColor((String)branchQRCodeSettingsMap.get("codeColor"));
        if (branchQRCodeSettingsMap.containsKey("backgroundColor")) branchQRCode.setBackgroundColor((String)branchQRCodeSettingsMap.get("backgroundColor"));
        if (branchQRCodeSettingsMap.containsKey("centerLogo")) branchQRCode.setCenterLogo((String)branchQRCodeSettingsMap.get("centerLogo"));
        if (branchQRCodeSettingsMap.containsKey("width")) branchQRCode.setWidth((Integer)branchQRCodeSettingsMap.get("width"));
        if (branchQRCodeSettingsMap.containsKey("margin")) branchQRCode.setMargin((Integer)branchQRCodeSettingsMap.get("margin"));
        
        if (branchQRCodeSettingsMap.containsKey("imageFormat")) {
            String imageFormat = (String)(branchQRCodeSettingsMap.get("imageFormat"));
            if (imageFormat != null ) {
    ...
    public static BranchEvent createBranchEvent(String eventName, ReadableMap params) {
        ...
        if (params.hasKey("currency")) {
            ...
            if (currency != null) {
                ...
                Log.w(NATIVE_SCRIPT_CLASS, "Invalid currency " + currencyString);
            }
        }
        ...
        if (params.hasKey("customData")) {
            ...
            while (it.hasNextKey()) {
                ...
                if(readableType == ReadableType.String) {
                    ...
                    Log.w(NATIVE_SCRIPT_CLASS, "customData values must be strings. Value for property " + key + " is not a string type.");
                }
            }
        }
    ...
    public BranchUniversalObject createBranchUniversalObject(ReadableMap branchUniversalObjectMap) {
            ...
            switch (branchUniversalObjectMap.getType("contentIndexingMode")) {
                case String:
                    ...
                    if (mode.equals("private"))
                        ...
                        Log.w(NATIVE_SCRIPT_CLASS, "Unsupported value for contentIndexingMode: " + mode +
                                ". Supported values are \"public\" and \"private\"");
                    break;
                default:
                    Log.w(NATIVE_SCRIPT_CLASS, "contentIndexingMode must be a String");
                    break;
            }
        }
        ...
        if (branchUniversalObjectMap.hasKey("expirationDate")) {
            ...
            try {
                ...
                Log.d(NATIVE_SCRIPT_CLASS, "Expiration date is " + date.toString());
                branchUniversalObject.setContentExpiration(date);
            }
            catch (ParseException e) {
                Log.w(NATIVE_SCRIPT_CLASS, "Invalid expiration date format. Valid format is YYYY-mm-ddTHH:MM:SS, e.g. 2017-02-01T00:00:00. All times UTC.");
            }
        }
    ...
    public ProductCategory getProductCategory(final String stringValue) {
        ...
        Log.w(NATIVE_SCRIPT_CLASS, "Could not find product category " + stringValue);
        return null;
    }

    public void sendNSEvent(String eventName, @Nullable WritableMap params) {
        ...
        Runnable poller = new Runnable() {
            ...
            private Runnable init(Context _context, Handler _mainHandler, String _eventName, WritableMap _params) {
            ...
            public void run() {
                try {
                    Log.d(NATIVE_SCRIPT_CLASS, "Catalyst instance poller try " + Integer.toString(tries));
                    if (mContext.hasActiveCatalystInstance()) {
                        Log.d(NATIVE_SCRIPT_CLASS, "Catalyst instance active");
                        mContext
                        ...
                        if (tries <= maxTries) {
                            ...
                            Log.e(NATIVE_SCRIPT_CLASS, "Could not get Catalyst instance");
                        }
                    }
                }
                catch (Exception e) {
        ...
        Log.d(NATIVE_SCRIPT_CLASS, "sendNSEvent");
    ...
    private static Map<String, Object> convertJsonToMap(JSONObject jsonObject) {
        ...
        try {
            ...
            map.putString("error", "Failed to convert JSONObject to Map: " + ex.getMessage());
        }

        return map;
    }

    private static List<Object> convertJsonToArray(JSONArray jsonArray) throws JSONException {
